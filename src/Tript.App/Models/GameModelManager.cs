...
internal sealed class GameModelManager : IDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var metadata = OnnxModelInspector.Inspect(Path.Combine(stagingPath, "model.onnx"));
            ...
            WriteJsonAtomic(Path.Combine(stagingPath, "installed.json"), new InstalledGameModel
            {
                ...
                ModelSha256 = package.Files["model.onnx"].Sha256,
    ...
    {
        var required = new HashSet<string>(["model.onnx", "events.json", "package.json"], StringComparer.Ordinal);
        using var archive = ZipFile.OpenRead(archivePath);
        long extractedBytes = 0;
        foreach (var entry in archive.Entries)
        {
            if (!required.Remove(entry.FullName) || entry.Name != entry.FullName)
                throw new InvalidDataException($"Unexpected model package entry '{entry.FullName}'.");
            extractedBytes = checked(extractedBytes + entry.Length);
            if (extractedBytes > MaximumExtractedBytes ||
                (entry.Name != "model.onnx" && entry.Length > 10 * 1024 * 1024))
            {
    ...
    {
        foreach (var name in new[] { "model.onnx", "events.json" })
        {
    ...
    private bool IsInstalledPackageHealthy(string gameId, InstalledGameModel installed)
    {
        if (!string.Equals(installed.GameId, gameId, StringComparison.OrdinalIgnoreCase) ||
            ...
            installed.ModelSha256.Length != 64 || installed.EventsSha256.Length != 64)
        {
            return false;
        }
        ...
        return File.Exists(modelPath) && File.Exists(eventsPath) &&
            string.Equals(HashFile(modelPath), installed.ModelSha256, StringComparison.OrdinalIgnoreCase) &&
