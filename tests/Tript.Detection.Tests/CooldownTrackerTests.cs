...
public class CooldownTrackerTests
{
    ...
    // Collects what the tracker writes, standing in for a real recording.
    ...
    // Readable from inside a CaptureBookmarks body, for assertions made mid-scenario.
    private static FakeRecordingSession? _activeRecording;

    // Bookmarks land on the active recording, which is the tracker's only externally visible
    // effect. The resolver is put back so the process-wide state is unchanged.
    private static List<Bookmark> CaptureBookmarks(Action body)
    {
        ...
        RecordingSessionRegistry.SetResolver(() => _activeRecording);
    ...
    public void ProcessDetection_SameBoxAfterLifetimeElapsed_CreatesSecondBookmark()
    {
        ...
        var bookmarks = CaptureBookmarks(() =>
        {
            ...
            Assert.Single(_activeRecording!.Bookmarks);
    ...
    public void Cleanup_ExpiresInstancesIndividually()
    {
        ...
        var bookmarks = CaptureBookmarks(() =>
        {
            ...
            Assert.Equal(2, _activeRecording!.Bookmarks.Count);
